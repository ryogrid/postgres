...
/*
実装の詳細は psql の実装（src/bin/psql/copy.c）を参考にして、
ecpg の特性に合わせて調整する必要があります。
*/
bool
ECPGcopy_from_stdin(const char *table, const char *column_list, bool csv_mode)
{
    //ECPGconn *conn = ECPGget_connection(NULL);
    PGresult *res;
    char *copy_cmd;
    char buffer[8192];
    char *line;
    ...
    copy_cmd = (char *) malloc(strlen("COPY ") + strlen(table) + 
                              strlen(" FROM STDIN") + 
                              (column_list ? strlen(column_list) + 2 : 0) + 
                              (csv_mode ? strlen(" CSV") : 0) + 1);
    
    if (!copy_cmd)
        return false;
        
    sprintf(copy_cmd, "COPY %s%s%s FROM STDIN%s",
            table,
            column_list ? " (" : "",
    ...
    /* issue copy command */
    res = PQexec(conn->connection, copy_cmd);
    free(copy_cmd);
    
    if (PQresultStatus(res) != PGRES_COPY_IN)
    {
        ecpg_raise(linenum, ECPG_PGSQL, NULL, PQerrorMessage(conn->connection));
        PQclear(res);
        return false;
    }
    ...
    while (fgets(buffer, sizeof(buffer), stdin) != NULL)
    {
        ...
        /* send data */
        if (PQputCopyData(conn->connection, buffer, strlen(buffer)) != 1)
        {
            ecpg_raise(linenum, ECPG_PGSQL, NULL, PQerrorMessage(conn->connection));
            PQclear(res);
            return false;
    ...
    /* finish copy */
    if (PQputCopyEnd(conn->connection, NULL) != 1)
    {
        ecpg_raise(linenum, ECPG_PGSQL, NULL, PQerrorMessage(conn->connection));
        PQclear(res);
        return false;
    ...
    {
        ecpg_raise(conn, ECPG_PGSQL, PQerrorMessage(conn->connection), line_num);
        PQclear(res);
        return false;
    }
